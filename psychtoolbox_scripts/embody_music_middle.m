 function embody_music_middle(subjectCode)
...
%     expStart; 
    ...
    outfilename = sprintf([pwd '/trialorder_music/%s_trialorder.mat'],subjectCode); 
    songOrder = load(outfilename);
    
    %Skip any trial that's already been done
    log = [pwd '/logfiles/' logfilename];
    formatSpec = '%f %s %s %f %f %s %s %s %s %s %s'; 
    fileID = fopen(log); 
    C = textscan(fileID,formatSpec,'commentstyle', '##'); 
    start = max(C{1});
    fprintf('Starting on trial number %d\n\n',start);
   
    %% Loop through each file: 
    for i = start:length(songOrder.songOrder)
        songfileName = songOrder.songOrder{i};
